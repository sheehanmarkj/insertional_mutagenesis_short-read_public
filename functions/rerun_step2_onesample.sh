...
$(basename $0) [-s sample-name] [-i path-to-data-dir] [-g cyno|mouse] [-r single|paired] [-h] usage info"
...
while getopts 's:i:g:r:' OPTION; do
        case "$OPTION" in
                s) sample="$OPTARG"
                printf -- "\033[32m The sample you've chosen to rerun is $OPTARG. \033[0m\n"
                ;;
                i) indir="$OPTARG"
...
if [ "x" == "x$sample" ]; then
  printf -- "\033[31m [-s sample-name] option is required. Please select the sample you want to rerun.\033[0m\n"
  exit
fi
...
## First remove any sample files from previous run attempt
...
if [ "$reads" == "paired" ]; then
 ...
 "${my_stools} view -f 68 -F 8 ${indir}/${sample}.bam | cut -f 1 | sort -T ${sort_dir} | uniq > ${mate_out}/${sample}_unaligned_reads_1_masked.txt
	...
	# Singly unmapped reads, unmasked alignment
 bsub -q medium -app large -J "v.int.mates_unmasked${sample}" -n 4,4 -M 16GB -R "span[hosts=1] rusage[mem=16GB]" \
 -o "${mate_out}/${sample}.v.int.2_unmasked.log" -e "${mate_out}/${sample}.v.int.2_unmasked.err" \
 "${my_stools} view -f 68 -F 8 ${indir}/${sample}_unmasked.bam | cut -f 1 | sort -T ${sort_dir} | uniq > ${mate_out}/${sample}_unaligned_reads_1_unmasked.txt
	...
	# Intersection of unaligned reads
 bsub -q medium -app large -w 'ended("v.int.mates*'${sample}'")' -J "v.int.mates_intersect${sample}" -n 4,4 -M 16GB -R "span[hosts=1] rusage[mem=16GB]" \
 -o "${mate_out}/${sample}.v.int.2_intersect.log" -e "${mate_out}/${sample}.v.int.2_intersect.err" \
 ...
 # Mates of unaligned reads - these are further subset downstream
