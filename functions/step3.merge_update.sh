...
$(basename $0) [-g cyno|mouse|human] [-i path-to-folder-containing-mate-and-softclip-folders] [-f path-to-folder-containing-initial-fastqs] [-r single|paired] [-w this is genewerk data] [-h] usage info"
...
while getopts 'g:i:f:r:w' OPTION; do
        case "$OPTION" in
                g) species="$OPTARG"
...
# first part is extracting the reads from viral bam
...
if [ "$reads" == "paired" ]; then
	...
	for f in $(ls -1 *_mapped_mates.bam); do
	nam=$(echo $f | sed 's/_mapped_mates\.bam//g')
	...
	nam=$(echo $f | sed 's/_softclipped\.bam//')
...
base=$(echo $f | sed 's/_softclipped\.bam//')
# Define temp sort directory
